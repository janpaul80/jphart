...
export default async function handler(req, res) {
    ...
    try {
        ...
        const LANGDOCK_ENDPOINT_URL = process.env.LANGDOCK_ENDPOINT_URL;

        if (!LANGDOCK_API_KEY || !LANGDOCK_ENDPOINT_URL) {
            console.error('Langdock credentials not found');
...
Then confirm you'll pass their info to Paul and he'll get back to them shortly.

Remember: Be helpful for questions about Paul's services. For anything else (debugging, how-to, etc.), kindly redirect to lead collection.`;
        ...
        const response = await fetch(`${LANGDOCK_ENDPOINT_URL}/chat/completions`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            console.error('Langdock API Error:', response.status, errorData);
            throw new Error(`Langdock API error: ${response.status}`);
        }

        const data = await response.json();
        const aiResponse = data.choices?.[0]?.message?.content ||
            "I apologize, but I couldn't generate a response. Please contact us directly.";
        ...
        return res.status(500).json({
            ...
            response: "I'm having a little trouble right now. Please reach out directly via WhatsApp (+593 98 9704265) or email (contact@paulhartmann.dev).",
            success: false
        });
    }
}
